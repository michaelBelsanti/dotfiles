...
/* harmony import */ var mustache__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(15);
...
function loadBlockedDomains() {
  Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_1__["includeReview"])(); // function to populate the paused domains list
  // send message to background to get blocked domains

  browser.runtime.sendMessage({
    'subject': 'getBlockedDomains'
  }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_1__["callbackErrorHandler"])(function (blockedDomains) {
    // render blocked domain data in a Mustache template
    var template = document.getElementById('domain-list-tmpl').innerHTML;
    mustache__WEBPACK_IMPORTED_MODULE_2___default.a.parse(template);
    var rendered = mustache__WEBPACK_IMPORTED_MODULE_2___default.a.render(template, {
      "blockedDomains": blockedDomains
    }); // put the rendered html in the domain list element

    document.getElementById('domain-list').innerHTML = rendered; // loop through all the elements and add an on click listener
    ...
    try {
      var _loop = function _loop() {
        ...
        domainRow.addEventListener('click', function (e) {
          e.preventDefault();
          var domain = domainRow.getAttribute('data-domain'); // are you sure pop-up on remove click, to prevent accidental clicks
          ...
          browser.runtime.sendMessage({
            ...
            'domain': domain
          }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_1__["callbackErrorHandler"])(function () {
            loadBlockedDomains();
          }));
        });
      };

      for (var _iterator = document.querySelectorAll('.domain-row')[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        _loop();
      }
    } catch (err) {
...
function loadPauseTimeout() {
  browser.runtime.sendMessage({
    'subject': 'getPauseTimeout'
  }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_1__["callbackErrorHandler"])(function (timeout) {
    document.getElementById("pause-timeout").value = timeout;
  }));
} // clear error and success messages


function cleanFormMessage(messageElementId) {
  var formMessageElement = document.getElementById(messageElementId);
...
window.onload = function () {
  loadBlockedDomains();
  loadPauseTimeout(); // on input change clean the form messages
  ...
  document.querySelector("#add-domain-form").addEventListener("submit", function (e) {
    e.preventDefault();
    cleanFormMessage('add-domain-form-message');
    var newDomain = document.getElementById('new-domain').value.trim().toLowerCase();
    ...
    browser.runtime.sendMessage({
      ...
      'domain': newDomain
    }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_1__["callbackErrorHandler"])(function (result) {
  ...
  var pauseTimeoutFormMessage;

  var savePauseTimeout = function savePauseTimeout() {
    var timeout = document.getElementById("pause-timeout").value;
    clearTimeout(pauseTimeoutFormMessage);
    cleanFormMessage('pause-timeout-form-message');
    browser.runtime.sendMessage({
      'subject': 'setPauseTimeout',
      'timeout': timeout
    }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_1__["callbackErrorHandler"])(function (result) {
      if (!result.error) {
        document.getElementById('pause-timeout-form-message').innerHTML = 'Pause timeout set to ' + timeout + ' seconds';
        document.getElementById('pause-timeout-form-message').classList.add('success'); // clear message after 5 seconds

        pauseTimeoutFormMessage = setTimeout(function () {
          cleanFormMessage('pause-timeout-form-message');
        }, 5000);
      } else {
        document.getElementById('pause-timeout-form-message').innerHTML = result.error;
        document.getElementById('pause-timeout-form-message').classList.add('error');
      }
    }));
  };

  document.getElementById("pause-timeout").addEventListener("keyup", function (e) {
    var newchar = String.fromCharCode(e.charCode || e.keyCode);

    if (!isNaN(newchar) || e.keyCode == 8) {
      savePauseTimeout();
    }
  });
  document.getElementById("pause-timeout").addEventListener("change", savePauseTimeout);
};
