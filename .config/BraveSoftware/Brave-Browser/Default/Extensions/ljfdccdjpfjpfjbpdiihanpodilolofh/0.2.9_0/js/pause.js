...
/* harmony import */ var _lib_util_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(14);
// this should not check current tab
...
var originalUrl = urlParams.get('url'); // track a page load for non-firefox browsers only

if (_settings_js__WEBPACK_IMPORTED_MODULE_1__["default"].BROWSER != 'firefox') {
...
var setPageName = function setPageName() {
  if (pageName) {
...
}; // ask the backend if it has a page name for the blocked URL


browser.runtime.sendMessage({
  'subject': 'getBlockedUrlName',
  'url': originalUrl
}, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_2__["callbackErrorHandler"])(function (blockedPageName) {
  pageName = blockedPageName;
  setPageName();
}));
browser.runtime.sendMessage({
  'subject': 'isUrlAllowed',
  'url': originalUrl
}, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_2__["callbackErrorHandler"])(function (isUrlAllowed) {
  if (isUrlAllowed) {
    window.location.replace(originalUrl);
  }
}));
browser.runtime.onMessage.addListener(Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_2__["callbackErrorHandler"])(function (msg, sender, response) {
...
$('document').ready(function () {
  ...
  setPageName();
  browser.runtime.sendMessage({
    'subject': 'incrementPausePageCounter'
  }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_2__["callbackErrorHandler"])(function (counter) {
    // show a feel good text every 5 page loads
    if (counter % 5 == 0) {
      $('.feel-good-text').removeClass('hidden');
    }
  }));
  ...
  $('#btn-allow-domain').on('click', function (e) {
    e.preventDefault(); // sensd a message to the background script to allow this URL for this tab

    browser.runtime.sendMessage({
      'subject': 'allowDomain',
      'url': originalUrl
    }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_2__["callbackErrorHandler"])(function () {
      // change the URL to the originalUrl URL once the message is received
      // and the callback is called from the background
      window.location.replace(originalUrl);
    }));
  });
  $('#btn-continue').on('click', function (e) {
    e.preventDefault(); // send a message to the background script to allow this URL for this tab

    browser.runtime.sendMessage({
      'subject': 'allowDomainOnce',
      'url': originalUrl
    }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_2__["callbackErrorHandler"])(function () {
      // change the URL to the original URL once the message is received
      // and the callback is called from the background
      window.location.replace(originalUrl);
    }));
  });
  ...
  if (_settings_js__WEBPACK_IMPORTED_MODULE_1__["default"].BROWSER == 'firefox') {
    document.getElementById('btn-back-wrapper').classList.remove('hidden');
  }

  setTimeout(function () {
    $('#freedom-logo').fadeIn(1000);
    $('#message-pause').fadeIn(1000);
  }, 500);
  setTimeout(function () {
    $('#freedom-logo').addClass('pulsate');
  }, 1500);
  browser.runtime.sendMessage({
    'subject': 'getPauseTimeout'
  }, Object(_lib_util_js__WEBPACK_IMPORTED_MODULE_2__["callbackErrorHandler"])(function (timeout) {
    if (!timeout) {
      timeout = 5;
    }

    if (timeout > 60) {
      $('#pause-timeout-desc').html('moment');
    }

    setTimeout(function () {
      $('#message-pause').fadeOut(400, function () {
        $('#message-pause').addClass('hidden');
        $('#message-ready').fadeIn(400);
        $('#message-ready').removeClass('hidden');
        setTimeout(function () {
          var extraButtonsWrapper = $('#extra-buttons-wrapper');
          extraButtonsWrapper.fadeIn(2000, function () {
            extraButtonsWrapper.removeClass('hidden');
          });
        }, 1000);
      });
    }, timeout * 1000 - 600 + 1500);
  }));
});
...
ga('create', _settings_js__WEBPACK_IMPORTED_MODULE_0__["default"].GOOGLE_ANALYTICS_ID, 'auto'); // Enter your GA identifier
// the following line allows to track chrome extension pages, that are not loaded through standard http requests
// this is the reason why we need to use the old google analytics with the ga function, 
// instead of the new gtag function
// there does not seem to be a way to do this with gtag
// code copied from: https://www.shanebart.com/chrome-ext-analytics/
